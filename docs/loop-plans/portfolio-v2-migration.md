...
### Phase A — Foundation
- [ ] A1. Add `css/tokens.css` + `css/grid.css` ported from reference tokens/globals (HTML, not React)
- [ ] A2. Add `js/mondrian-lines.js` — port bend logic from `cca3c36:site/components/MondrianLines.tsx` + `site/lib/motion.ts`
...
- [ ] B8. `index.html` *(homepage — Webflow content in new shell)*
...
- Loop plan created. No migration work started yet.
- Tick interval: **5m**. Concurrency lock at `.loop/portfolio-v2.lock` (10m stale).
- Design reference: `claude/new-branch-oys0jm` @ `cca3c36` — port UI to HTML only.
- First task when automation starts: **A1** (CSS tokens + grid foundation).
- Homepage copy decision pending: Webflow verbatim vs Mondrian shell copy — default to **Webflow verbatim** until Priyal says otherwise.
