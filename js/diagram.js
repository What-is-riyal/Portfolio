/**
 * Hero diagram — vanilla JS port of cca3c36:site/components/Diagram.tsx
 *
 * Usage:
 *   const svg = Diagram.mount(document.querySelector('.hero-diagram'));
 *   Diagram.init(svg);
 *
 * Or: Diagram.mountAndInit(container)
 *
 * Requires GSAP and Motion (js/motion.js).
 */
(function (global) {
  const CX = 210;
  const CY = 214;
  ...
  function svgMarkup() {
    return `
<svg viewBox="0 0 420 420" width="100%" role="img"
  ...
  <g data-hover="ring">
    <g data-drift="ring">
      <circle cx="${CX}" cy="${CY}" r="196" fill="none" stroke="var(--ink)" stroke-opacity="0.45"
        stroke-width="1.5" stroke-dasharray="1 8" stroke-linecap="round"/>
    </g>
  </g>
  <g data-hover="people">
    <g data-drift="people">
      <circle cx="158" cy="${CY}" r="112" fill="none" stroke="var(--ink)" stroke-opacity="0.7" stroke-width="1.5"/>
    </g>
  </g>
  <g data-hover="tech">
    <g data-drift="tech">
      <circle cx="262" cy="${CY}" r="112" fill="none" stroke="var(--ink)" stroke-opacity="0.7" stroke-width="1.5"/>
    </g>
  </g>
  ...
  function mountAndInit(container) {
    const svg = mount(container);
    return init(svg);
  }

  global.Diagram = { mount, init, mountAndInit, svgMarkup };
