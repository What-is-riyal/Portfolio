...
(function () {
  ...
  function bootHumanField() {
    const canvas = document.querySelector('[data-human-field]');
    const hero = canvas ? canvas.closest('.home-hero') : null;
    ...
    const draw = (time) => {
      ...
      const live = points.map((point) => {
        ...
        const y = point.y * height + Math.cos(t * point.speed * 0.8 + point.phase) * 14;
      ...
      for (let a = 0; a < live.length; a += 1) {
        for (let b = a + 1; b < live.length; b += 1) {
          ...
          context.strokeStyle = `rgba(84, 70, 184, ${((1 - distance / 104) * 0.09).toFixed(3)})`;
      ...
      live.forEach((point, index) => {
        ...
        context.fillStyle = index % 5 === 0 ? 'rgba(84, 70, 184, 0.28)' : 'rgba(30, 29, 34, 0.17)';
        context.fill();
      });
